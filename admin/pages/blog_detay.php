<?php 
if(!isset($ACCESS)){
	die("ERROR!");
}

if ((isset($_GET['ID'])) && ($_GET['ID'] != "") && (isset($_GET['islemsil']))) {
  $deleteSQL = sprintf("DELETE FROM tablo_blog WHERE ID=%s",
                       escape($_GET['ID'], "int"));
  $Result1 = mysqli_query($Conn, $deleteSQL) or die(mysqli_error());
  admin_cevirileri_sil('blog', $_GET['ID']);
  seoURLSil("blog_detay",$_GET['ID']);
  $Url = $AdminURL."index.php?sayfa=blog";	
  yonlendir_($Url);
}

if ((isset($_POST["islem"])) && ($_POST["islem"] == "kaydet")) {
	
	$Resim = upload("../uploads/","Resim","ResimEski");
	$BuyukResim = upload("../uploads/","BuyukResim","BuyukResimEski");
				
  $insertSQL = sprintf("INSERT INTO tablo_blog (Resim, BuyukResim, Baslik, KisaBilgi, Detay, Tarih, SiraNo, Slider, Kategori) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)",
                       escape($Resim, "text"),
                       escape($BuyukResim, "text"),
                       escape($_POST['Baslik'], "text"),
                       escape($_POST['KisaBilgi'], "text"),
                       escape($_POST['Detay'], "text"),
                       escape($_POST['Tarih'], "text"),
                       escape($_POST['SiraNo'], "int"),
                       escape($_POST['Slider'], "text"),
					   escape($_POST['Kategori'], "text"));
  $Result1 = mysqli_query($Conn, $insertSQL) or die(mysqli_error());
  
  $kayitID = mysqli_insert_id($Conn);
  seoURLKaydet("blog_detay",$kayitID,$_POST['Baslik'],$_POST['Description']);
  admin_cevirileri_kaydet('blog', $kayitID);
  admin_url_cevirilerini_kaydet('blog_detay', $kayitID);
    
  $Url = $AdminURL."index.php?sayfa=blog";	
  yonlendir_($Url);
}

if ((isset($_POST["islem"])) && ($_POST["islem"] == "guncelle")) {
	
	$Resim = upload("../uploads/","Resim","ResimEski");
	$BuyukResim = upload("../uploads/","BuyukResim","BuyukResimEski");
		
  $updateSQL = sprintf("UPDATE tablo_blog SET Resim=%s, BuyukResim=%s, Baslik=%s, KisaBilgi=%s, Detay=%s, Tarih=%s, SiraNo=%s, Slider=%s, Kategori=%s WHERE ID=%s",
                       escape($Resim, "text"),
                       escape($BuyukResim, "text"),
                       escape($_POST['Baslik'], "text"),
                       escape($_POST['KisaBilgi'], "text"),
                       escape($_POST['Detay'], "text"),
                       escape($_POST['Tarih'], "text"),
                       escape($_POST['SiraNo'], "int"),
                       escape($_POST['Slider'], "text"),
					   escape($_POST['Kategori'], "text"),
                       escape($_POST['ID'], "int"));
  $Result1 = mysqli_query($Conn, $updateSQL) or die(mysqli_error());
  
  seoURLKaydet("blog_detay",$_POST['ID'],$_POST['Baslik'],$_POST['Description']); 
  admin_cevirileri_kaydet('blog', $_POST['ID']);
  admin_url_cevirilerini_kaydet('blog_detay', $_POST['ID']);
   
    
  $Url = $AdminURL."index.php?sayfa=blog";	
  yonlendir_($Url);
}

$ID = "-1";
if (isset($_GET['ID'])) {
  $ID = $_GET['ID'];
}

$query_rsDetay = sprintf("SELECT * FROM tablo_blog WHERE ID = %s", escape($ID, "int"));
$rsDetay = mysqli_query($Conn, $query_rsDetay) or die(mysqli_error());
$row_rsDetay = mysqli_fetch_assoc($rsDetay);
$totalRows_rsDetay = mysqli_num_rows($rsDetay);

$query_rsSeo = sprintf("SELECT * FROM tablo_url WHERE Sayfa = 'blog_detay' AND ID = %s", escape($ID, "int"));
$rsSeo = mysqli_query($Conn, $query_rsSeo) or die(mysqli_error());
$row_rsSeo = mysqli_fetch_assoc($rsSeo);
$totalRows_rsSeo = mysqli_num_rows($rsSeo);
?>
<?php  $SayfaTitle = "Blog"; ?>
<div class="content-wrapper">
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1><?php echo $SayfaTitle ?></h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="<?php echo sayfa("anasayfa") ?>">Anasayfa</a></li>
              <li class="breadcrumb-item active"><?php echo $SayfaTitle ?></li>
            </ol>
          </div>
        </div>
      </div>
    </section>
    <section class="content">
      <div class="container-fluid">
        <div class="row">
          
          <!-- /.col -->
          <div class="col-md-12">
            <div class="card card-primary card-outline">
            
            <div class="card-header">
          <h3 class="card-title"><?php echo $SayfaTitle ?></h3>
        </div>
              <!-- /.card-header -->
              <div class="card-body">
                <div class="tab-content">
                <div class="tab-pane active" id="bilgilerim">
                    <form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
                      
                     <div class="form-group row">
                        <label class="col-sm-2 col-form-label">Resim</label>
                        <div class="col-sm-10">
                              <div class="custom-file">
                                	<input type="file" class="custom-file-input" name="Resim" id="Resim">
                                	<label class="custom-file-label" for="Resim">Resim Seç</label>
                                </div>
                        </div>
                      </div>
                      
                      <!--<div class="form-group row">
                        <label class="col-sm-2 col-form-label">Sağ Resim</label>
                        <div class="col-sm-10">
                              <div class="custom-file">
                                	<input type="file" class="custom-file-input" name="BuyukResim" id="BuyukResim">
                                	<label class="custom-file-label" for="BuyukResim">Resim Seç</label>
                                </div>
                        </div>
                      </div> -->
                      
                      <div class="form-group row">
                        <label class="col-sm-2 col-form-label">Başlık</label>
                        <div class="col-sm-10">
                          <input name="Baslik" type="text" required class="form-control" value="<?php echo $row_rsDetay['Baslik']; ?>">
                        </div>
                      </div>
                      
                      <div class="form-group row">
                        <label class="col-sm-2 col-form-label">Tarih</label>
                        <div class="col-sm-10">
                          <input name="Tarih" type="date" required class="form-control" value="<?php echo $row_rsDetay['Tarih']; ?>">
                        </div>
                      </div>
                      
                      <div class="form-group row">
                        <label class="col-sm-2 col-form-label">KisaBilgi</label>
                        <div class="col-sm-10">
                          <textarea name="KisaBilgi" class="form-control"><?php echo $row_rsDetay['KisaBilgi']; ?></textarea>
                        </div>
                      </div>
                      
                      <div class="form-group row">
                        <label class="col-sm-2 col-form-label">Detay</label>
                        <div class="col-sm-10">
                          <textarea name="Detay" class="form-control editor"><?php echo $row_rsDetay['Detay']; ?></textarea>
                        </div>
                      </div>
                      
                  
                      <!--<div class="form-group row">
                        <label class="col-sm-2 col-form-label">SiraNo</label>
                        <div class="col-sm-10">
                          <input name="SiraNo" type="text" class="form-control" placeholder="SiraNo" value="<?php echo $row_rsDetay['SiraNo']; ?>">
                        </div>
                      </div> -->
                      
                     
                     <div class="form-group row">
                        <label class="col-sm-2 col-form-label">Slayt Gösterisine Ekle?</label>
                        <div class="col-sm-10">
                        
                        <select class="form-control" name="Slider">
                            <option <?php if ($row_rsDetay['Slider']=="Evet"){echo " selected";} ?>>Evet</option>
                            <option <?php if ($row_rsDetay['Slider']=="Hayir"){echo " selected";} ?>>Hayir</option>
                          </select>
                        
                         
                        </div>
                      </div>
                      
                      <!-- 
                      <div class="form-group row">
                        <label class="col-sm-2 col-form-label">SEO Description</label>
                        <div class="col-sm-10">
                          <input name="Description" type="text" class="form-control" value="<?php echo $row_rsSeo['Description']; ?>"/>
                        </div>
                      </div> -->
                      
                      
                      
                      <?php admin_ceviri_sekmeleri('blog', $ID, array(
                        'Baslik' => 'Başlık',
                        'KisaBilgi' => array('Kısa Bilgi', 'textarea'),
                        'Detay' => array('Detay', 'editor'),
                        'Kategori' => 'Kategori'
                      ), 'blog_detay'); ?>

                      <div class="form-group row">
                        <div class="offset-sm-2 col-sm-10">
                          <button type="submit" class="btn btn-danger">Kaydet</button>
                        </div>
                      </div>
                        <?php if ($totalRows_rsDetay>0){ ?>
                            <input type="hidden" name="islem" value="guncelle">
                        <?php }else { ?>
                        	<input type="hidden" name="islem" value="kaydet">
                        <?php }?>
                        <input type="hidden" name="ID" value="<?php echo $row_rsDetay['ID']; ?>">
                        <input type="hidden" name="ResimEski" value="<?php echo $row_rsDetay['Resim']; ?>">
                        <input type="hidden" name="BuyukResimEski" value="<?php echo $row_rsDetay['BuyukResim']; ?>">
                        
                        <input type="hidden" name="Link" value="<?php echo $row_rsDetay['Link']; ?>">
                    </form>
                  </div>
                <!-- /.tab-pane --><!-- /.tab-pane -->

                  
                  <!-- /.tab-pane -->
                </div>
                <!-- /.tab-content -->
              </div><!-- /.card-body -->
            </div>
            <!-- /.nav-tabs-custom -->
          </div>
          <!-- /.col -->
        </div>
        <!-- /.row -->
      </div><!-- /.container-fluid -->
    </section>
  </div>
